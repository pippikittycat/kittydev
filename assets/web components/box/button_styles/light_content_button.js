...
createComponent({
  tagName: 'light-content-box',
  attributes: ['text', 'svg', 'url-link'],
  stylesheet: styles,
  render: ({text, urlLink, svg}) => {return `
    <div>
    ${urlLink ? `<a href="${urlLink}" class="no-text-decoration fit-content flex row soft-pillow-bg rounded-corners large-padding border arial-nova svg-gap light-button brown-sugar">` : ''}
${svg ? `${svg}` : ''}
${text ? `<p>${text}</p>` : ''}
</a>
</div>
